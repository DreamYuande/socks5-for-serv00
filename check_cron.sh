...
REBOOT_XRAYR="pgrep -x \"XrayR\" >/dev/null || nohup ${XRAYR_PATH}/XrayR --config ${XRAYR_CONFIG} >/dev/null 2>&1 &"

echo "检查并添加 crontab 任务"

if [ "$(command -v pm2)" == "/home/${USER}/.npm-global/bin/pm2" ]; then
  echo "已安装 pm2，并返回正确路径，启用 pm2 保活任务"
  (crontab -l | grep -F "@reboot $PM2_PATH resurrect >> /home/$(whoami)/pm2_resurrect.log 2>&1") || \
  (crontab -l; echo "@reboot $PM2_PATH resurrect >> /home/$(whoami)/pm2_resurrect.log 2>&1") | crontab -
else
  if [ -e "${WORKDIR}/start.sh" ] && [ -e "${FILE_PATH}/config.json" ]; then
    echo "添加 nezha & socks5 的 crontab 重启任务"
    (crontab -l | grep -F "@reboot ${CRON_S5} && ${CRON_NEZHA}") || \
    (crontab -l; echo "@reboot ${CRON_S5} && ${CRON_NEZHA}") | crontab -
    ...
    (crontab -l | grep -F "@reboot ${CRON_NEZHA}") || \
    (crontab -l; echo "@reboot ${CRON_NEZHA}") | crontab -
    (crontab -l | grep -F "*/12 * * * * pgrep -x \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") || \
    (crontab -l; echo "*/12 * * * * pgrep -x \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") | crontab -
  elif [ -e "${FILE_PATH}/config.json" ]; then
    echo "添加 socks5 的 crontab 重启任务"
    (crontab -l | grep -F "@reboot ${CRON_S5}") || \
    (crontab -l; echo "@reboot ${CRON_S5}") | crontab -
    (crontab -l | grep -F "*/12 * * * * pgrep -x \"s5\" > /dev/null || ${CRON_S5}") || \
    (crontab -l; echo "*/12 * * * * pgrep -x \"s5\" > /dev/null || ${CRON_S5}") | crontab -
  fi
fi
...
if [ -x "${XRAYR_PATH}/XrayR" ] && [ -f "${XRAYR_CONFIG}" ]; then
  ...
  # 添加系统重启时启动任务
  (crontab -l | grep -F "@reboot ${REBOOT_XRAYR}") || \
  (crontab -l; echo "@reboot ${REBOOT_XRAYR}") | crontab -
